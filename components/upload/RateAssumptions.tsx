...
const VALUE_PRESETS = [
  { label: "< 1M", value: 500_000 },
  { label: "1–5M", value: 2_500_000 },
  { label: "5–20M", value: 10_000_000 },
  { label: "20–100M", value: 50_000_000 },
  { label: "> 100M", value: 150_000_000 },
];
...
export function RateAssumptions({ ctx, onChange, defaultOpen = false }: Props) {
  ...
  const activeValuePreset = VALUE_PRESETS.find((p) => p.value === ctx.estimatedValueZMW);
    ...
    <div className="rounded-lg border border-white/10 bg-white/[0.02]">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-white/5">
          ...
          {/* Government tender toggle */}
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-white">Procurement type</p>
            <div className="flex gap-1.5">
              {[
                { val: false, label: "Private / commercial" },
                { val: true, label: "Government tender" },
              ].map(({ val, label }) => (
                <button
                  key={String(val)}
                  type="button"
                  onClick={() => set({ isGovernmentTender: val })}
                  className={`flex-1 px-3 py-2 rounded text-xs font-medium transition-colors text-center ${
                    ctx.isGovernmentTender === val
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-white">
              Estimated project value <span className="text-gray-500 font-normal">(ZMW, helps price P&G)</span>
            </p>
            <div className="flex flex-wrap gap-1.5">
              {VALUE_PRESETS.map(({ label, value }) => (
                <button
                  key={label}
                  type="button"
                  onClick={() => set({ estimatedValueZMW: ctx.estimatedValueZMW === value ? null : value })}
                  className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                    ctx.estimatedValueZMW === value ? "bg-amber-400 text-black" : "bg-white/10 text-gray-300 hover:bg-white/15"
                  }`}
                >
                  {label}
                </button>
            ...
            {ctx.estimatedValueZMW && !activeValuePreset && (
              <p className="text-[11px] text-gray-500">ZMW {ctx.estimatedValueZMW.toLocaleString()}</p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              {[
                { val: "main_road", label: "Main road", sub: "Standard transport costs" },
                { val: "gravel_road", label: "Gravel / secondary road", sub: "+10–20% on materials" },
                { val: "remote", label: "Remote / bush site", sub: "+25–40% on materials" },
              ].map(({ val, label, sub }) => (
                <button
                  key={val}
                  type="button"
                ...
                >
                  ...
                  <span className="text-[11px] text-gray-500 ml-auto">{sub}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              {[
                { val: "local_unskilled", label: "Mostly local unskilled", sub: "Use lower end of rates" },
                { val: "mixed", label: "Mix of skilled & unskilled", sub: "Use mid-range rates" },
                { val: "imported_skilled", label: "Imported / specialist trades", sub: "Use upper end of rates" },
              ].map(({ val, label, sub }) => (
                <button
                  key={val}
                  type="button"
