...
export function findRateAnchors(
  ...
  const historicalScored = tokenizedHistorical
    .map(({ entry, tokens, normUnit }) => {
      ...
      // Boost same project_type entries by 15%
      const typeBoost = projectType && entry.project_type && entry.project_type === projectType ? 1.15 : 1.0;
      const score = descScore * unitMatch * typeBoost;
